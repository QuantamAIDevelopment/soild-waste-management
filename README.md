...
EXTERNAL_UPLOAD_URL=http://192.168.100.154:8000/api/vehicle-routes/upload-data
```
...
curl -X POST "http://localhost:8000/assign-routes-by-vehicle" \
  -H "Authorization: Bearer swm-2024-secure-key" \
  -F "vehicle_ids=SA001,SA002,SA003"

# Step 3: Get routes and auto-upload
curl -X GET "http://localhost:8000/cluster-routes" \
  -H "Authorization: Bearer swm-2024-secure-key"
```
...
docker run -p 8000:8000 --env-file .env swm-optimizer
```
